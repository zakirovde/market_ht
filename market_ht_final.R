...
# тээк чет есть одно бракованное значение (в итоге не пригодилось)
which(rfm$geo_id=='$s')
# просто поменяю его на самое распространенное - 10000
head(as.data.frame(table(rfm$geo_id)) %>% arrange(desc(Freq)), 1)
...
ggplot(filter(monetary, money>0), aes(money)) + geom_histogram() + xlim(0,8000)
...
clients_urfm <- rfm %>% group_by(user_id) %>% summarise(money=sum(money), rf=min(rf))
...
rmarkdown::render('market_ht.Rmd')
